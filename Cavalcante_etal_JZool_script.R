...
jaccard.test.bootstrap(var1, var2, B=1000)#bootstrap with 1000 iterations 

##### Simulations to identify the type I error rate #####

#Example with N= 366 days
#Note that you can use your own sample size (number of days) to test #the feasibility of this test for your study
p = NA

for(i in 1:100){
  x1 <- sample(c(0,1), 366, replace=TRUE)
  x2 <- sample(c(0,1), 366, replace=TRUE)
